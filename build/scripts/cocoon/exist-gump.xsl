...
<xsl:transform version="1.0"
	...
	<!-- Patch Cocoon's gump.xml file. Add project definition for the eXist 
		block. We also need to patch the xmldb block as it uses an incompatible XML:DB 
		version. -->
		...
		<module>
			...
			<xsl:if test="not(project[@name='cocoon-block-exist'])">
				<project name="cocoon-block-exist" status="stable" dir="src/blocks/exist">
					...
					<!-- RvdB: patch to include missing eXist libs in Cocoon build (otherwise 
						Cocoon build won't copy them to /build/WEB-INF/lib) -->
					<library name="ws-commons" />
					<library name="sunxacml" />
					<library name="jing" />
					<library name="isorelax" />
					<library name="jaxrpc" />
					<library name="saaj" />
					<library name="pkg-repo" />
				</project>
			</xsl:if>
		</module>
	</xsl:template>
