...
public class Predicate extends PathExpr {
	...
	public void analyze(AnalyzeContextInfo contextInfo) throws XPathException {
		...
		final int innerType = staticReturnType != Type.ITEM ? staticReturnType
				: inner.returnsType();
		...
		if (Type.subTypeOf(innerType, Type.NODE)
			...
			// dependency with the context item.
				...
				&& !Dependency.dependsOn(inner, Dependency.CONTEXT_ITEM)
				&& Cardinality.checkCardinality(inner.getCardinality(),
						Cardinality.EXACTLY_ONE))
			executionMode = POSITIONAL;
		// Case 3: all other cases, boolean evaluation (that can be "promoted"
		// later)
		else
	...
	private AnalyzeContextInfo createContext(AnalyzeContextInfo contextInfo) {
		...
		newContextInfo.addFlag(IN_PREDICATE); // set flag to signal
												// subexpression that we are in
												// a predicate
		...
		if (inner == null)
			...
			if (Type.subTypeOf(contextSequence.getItemType(), Type.ATOMIC)) {
				...
				if (executionMode == NODE
					...
					// (1,2,2,4)[.]
					if (Type.subTypeOf(contextSequence.getItemType(),
							Type.NUMBER)) {
				...
				if (executionMode == BOOLEAN
						&& !Dependency
								.dependsOn(inner, Dependency.CONTEXT_ITEM)
					...
					if (innerSeq.hasOne()
							&& Type.subTypeOf(innerSeq.getItemType(),
									Type.NUMBER)) {
						recomputedExecutionMode = POSITIONAL;
					}
				}
			} else if (executionMode == NODE
				...
				if (executionMode == BOOLEAN
					...
					if (Type.subTypeOf(innerSeq.getItemType(), Type.NODE)
						...
						// Try to promote a boolean evaluation to a positional
						// one
						// Only if we have an actual *singleton* of numeric
						// items
...
//TODO:				result = selectByPosition(outerSequence, contextSequence, mode,
//						innerSeq);
				break;
			default:
				throw new IllegalArgumentException(
						"Unsupported execution mode: '"
								+ recomputedExecutionMode + "'");
			}
		}
	...
	public Sequence evalPredicate(Sequence outerSequence,
		...
		if (inner == null)
			...
			case POSITIONAL:
				...
				result = selectByPosition(outerSequence, contextSequence, mode,
						innerSeq);
				break;
			default:
				throw new IllegalArgumentException(
	 ...
	 * @throws XPathException
	 */
	private Sequence selectByNodeSet(Sequence contextSequence)
			throws XPathException {
		 ...
		 * NodeSet nodes; if (contextIsVirtual) { ArraySet copy = new
		 * ArraySet(contextSet.getLength()); for (Iterator i =
		 * contextSet.iterator(); i.hasNext();) { copy.add((Item)i.next()); }
		 * nodes = super.eval(copy, null).toNodeSet(); } else nodes =
		 * super.eval(contextSet, null).toNodeSet(); //End of work-around
		 */
		 ...
		 */
		if (cached != null && cached.isValid(contextSequence, null)
				&& nodes.isCached()) {
			if (context.getProfiler().isEnabled())
		...
		if (outerSequence != null && !outerSequence.isEmpty()
			...
			case Constants.DESCENDANT_ATTRIBUTE_AXIS: {
				...
				if (outerSequence instanceof VirtualNodeSet) {
					...
					// Looks like we don't need to treat VirtualNodeSet
					// specially here.
				...
				// Comment the line below if you have uncommented the lines
				// above :-)
				...
				if (contextSet.getDocumentSet().intersection(
					...
					LOG
							.info("contextSet and outerNodeSet don't share any document");
				...
				for (SequenceIterator i = ancestors.iterate(); i.hasNext();) {
					...
					for (SequenceIterator j = innerSeq.iterate(); j.hasNext();) {
						...
						if (!v.hasFractionalPart() && !v.isZero()) {
							...
							// TODO : rename this method as getInDocumentOrder ?
							// -pb
							p = temp.get(v.getInt() - 1);
							if (p != null) {
								// Commented out : but this is probably more
								// complicated (see test case in the same
								// commit)
								// p.clearContext(Expression.IGNORE_CONTEXT);
								result.add(p);
							}
							// TODO : does null make sense here ? Well...
							// sometimes ;-)
						}
					}
				}
			...
			default:
				for (SequenceIterator i = outerSequence.iterate(); i.hasNext();) {
					...
					case Constants.ANCESTOR_AXIS:
						temp = contextSet.selectAncestors(p, false,
								Expression.IGNORE_CONTEXT);
						break;
					case Constants.ANCESTOR_SELF_AXIS:
						temp = contextSet.selectAncestors(p, true,
								Expression.IGNORE_CONTEXT);
						break;
					...
					case Constants.PRECEDING_SIBLING_AXIS:
						temp = contextSet.selectPrecedingSiblings(p,
								Expression.IGNORE_CONTEXT);
						break;
					case Constants.FOLLOWING_SIBLING_AXIS:
						temp = contextSet.selectFollowingSiblings(p,
								Expression.IGNORE_CONTEXT);
						reverseAxis = false;
						break;
					...
					case Constants.SELF_AXIS:
						...
						break;
					default:
						// temp = contextSet.selectAncestorDescendant(p,
						// NodeSet.DESCENDANT, false, false);
						// break;
						throw new IllegalArgumentException(
								"Tested unknown axis");
					}
					if (!temp.isEmpty()) {
						for (SequenceIterator j = innerSeq.iterate(); j
								.hasNext();) {
							NumericValue v = (NumericValue) j.nextItem();
							// Non integers return... nothing, not even an error
							// !
							if (!v.hasFractionalPart() && !v.isZero()) {
								int pos = (reverseAxis ? temp.getItemCount()
										- v.getInt() : v.getInt() - 1);
