...
public class OpNumeric extends BinaryOp {
    ...
    public Sequence eval(Sequence contextSequence, Item contextItem) throws XPathException {
        ...
        if (lseq.hasMany())
            throw new XPathException(this, ErrorCodes.XPTY0004, "too many operands at the left of " + Constants.OPS[operator]);
        if (rseq.hasMany())
            throw new XPathException(this, ErrorCodes.XPTY0004, "too many operands at the right of " + Constants.OPS[operator]);
        ...
        if (rseq.isEmpty())
            ...
            try {
                ...
                if (!(lvalue instanceof ComputableValue))
                    throw new XPathException(this, ErrorCodes.XPTY0004, "'" + Type.getTypeName(lvalue.getType()) + "(" + lvalue + ")' can not be an operand for " + Constants.OPS[operator]);
                if (!(rvalue instanceof ComputableValue))
                    throw new XPathException(this, ErrorCodes.XPTY0004, "'" + Type.getTypeName(rvalue.getType()) + "(" + rvalue + ")' can not be an operand for " + Constants.OPS[operator]);
		...
		switch (operator) {
			...
			case Constants.MOD:		{
				if (!Type.subTypeOf(left.getType(), Type.NUMBER))
    				throw new XPathException(this, ErrorCodes.XPTY0004, "'" + Type.getTypeName(left.getType()) + "(" + left + ")' is not numeric");
				if (!Type.subTypeOf(right.getType(), Type.NUMBER))
    				throw new XPathException(this, ErrorCodes.XPTY0004, "'" + Type.getTypeName(right.getType()) + "(" + right + ")' is not numeric");
