...
/**
 * Allow's the TransformerFactory that is used for XSLT to be
...
{
	/* 
	 * problem is that even when a Xalan is defined in conf.xml, if Saxon JAR's are on the classpath then
	 * when we create a Xalan TransformerFactory and newInstance() is called on it, it returns a Saxon TransformerFactory :-(
	 */
	
	private final static Logger LOG = Logger.getLogger(TransformerFactoryAllocator.class);	//Logger
	...
	public static SAXTransformerFactory getTransformerFactory(DBBroker broker)
    {
    	...
    	String transformerFactoryClassName = (String)broker.getConfiguration().getProperty("transformer.class");
    
    	System.out.println("transformerFactoryClassName=" + transformerFactoryClassName);
        ...
        else
        {
            ...
            {
                LOG.debug( "Error found loading the requested TrAX Transformer Factory '" + transformerFactoryClassName + "'. Using default TrAX Transformer Factory instead." );
