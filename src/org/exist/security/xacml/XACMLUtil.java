...
public class XACMLUtil implements UpdateListener
{
	...
	private static final String[] samplePolicyDocs = {"/xacml/builtin_policy.xml", "/xacml/external_modules_policy.xml", "/xacml/reflection_policy.xml"};
	...
	XACMLUtil(ExistPDP pdp)
	{
		...
		pool.getNotificationService().subscribe(this);
		initializePolicyCollection(pool);
	}
	private void initializePolicyCollection(BrokerPool pool)
	{
	...
	private void initializePolicyCollection(DBBroker broker)
	{
		...
		{
			Configuration conf = broker.getConfiguration();
			if(conf == null || conf.getProperty(XACMLConstants.LOAD_DEFAULT_POLICIES_PROPERTY) == Boolean.TRUE)
	...
	*/
	...
	public static Collection getPolicyCollection(DBBroker broker)
	{
		...
		if(policyCollection == null)
		{
			LOG.debug("Creating policy collection '" + XACMLConstants.POLICY_COLLECTION + "'...");
