...
public class NodeIDSet extends NodeSet {
    ...
    private static Category LOG = Category.getInstance(NodeIDSet.class.getName());
    protected ArrayList list = new ArrayList();
    protected TreeSet set;
    //protected BTree set = new BTree(1000);
    
    public NodeIDSet() {
	super();
	set = new TreeSet(NodeProxy.NodeProxyComparator.instance);
    }
	...
	NodeProxy p =
	    new NodeProxy(((NodeImpl)node).ownerDocument, ((NodeImpl)node).gid);
	add(p);
    }
  ...
  public Iterator iterator() {
   ...
   public int position(NodeImpl node) {
      NodeProxy p = new NodeProxy(node.ownerDocument,
				  node.getGID());
