...
public class DocumentImpl extends NodeImpl implements Document, Comparable {
     ...
     */
    protected static NodeImpl createNode( long gid, short type,
                                          String name, String data, int children ) {
        NodeImpl node;
        switch ( type ) {
            case Node.TEXT_NODE:
    ...
    public void calculateTreeLevelStartPoints() {
        ...
        for ( int i = 1; i < maxDepth - 1; i++ )
            treeLevelStartPoints[i + 1] = ( treeLevelStartPoints[i] -
                treeLevelStartPoints[i - 1] ) * treeLevelOrder[i] +
     ...
     *@return          Description of the Return Value
     */
    protected NodeList findElementsByTagName( NodeImpl root,
                                              String tagName ) {
        ...
        context.add( root );
        NodeSet temp =
            (NodeSet) broker.findElementsByTagName( docs, tagName );
        NodeProxy p;
    ...
    public int getTreeLevel( long gid ) {
        for ( int i = 0; i < maxDepth; i++ ) {
            if ( ( gid >= treeLevelStartPoints[i] ) && ( i + 1 == maxDepth ||
