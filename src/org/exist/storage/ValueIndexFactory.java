...
public class ValueIndexFactory {
	...
	public static int OFFSET_VALUE = OFFSET_TYPE + ValueIndexFactory.LENGTH_TYPE; //3	
	...
	public final static Indexable deserialize(byte[] data, int start, int len) throws EXistException {
		...
		if (Type.subTypeOf(type, Type.STRING))
		{
			String s;
			try {
				s = new String(data, start + (Collection.LENGTH_COLLECTION_ID + ValueIndexFactory.LENGTH_TYPE),
				len - (Collection.LENGTH_COLLECTION_ID + ValueIndexFactory.LENGTH_TYPE), "UTF-8");
		...
		/* xs:dateTime */
			...
			long value = ByteConversion.byteToLong(data, start + (Collection.LENGTH_COLLECTION_ID + ValueIndexFactory.LENGTH_TYPE));			
		...
		{
			return new IntegerValue(ByteConversion.byteToLong(data, start + (Collection.LENGTH_COLLECTION_ID + ValueIndexFactory.LENGTH_TYPE)) ^ 0x8000000000000000L);
		}
		...
		{
			long bits = ByteConversion.byteToLong(data, start + (Collection.LENGTH_COLLECTION_ID + ValueIndexFactory.LENGTH_TYPE)) ^ 0x8000000000000000L;
		...
		{
			int bits = ByteConversion.byteToInt(data, start + (Collection.LENGTH_COLLECTION_ID + ValueIndexFactory.LENGTH_TYPE)) ^ 0x80000000;
		...
		{
			return new BooleanValue(data[start + (Collection.LENGTH_COLLECTION_ID + ValueIndexFactory.LENGTH_TYPE)] == 1);
		}		
	...
	public final static byte[] serialize(Indexable value, short collectionId, boolean caseSensitive) 
		...
		{			
			...
			final byte[] data = new byte[UTF8.encoded(val) + (Collection.LENGTH_COLLECTION_ID + ValueIndexFactory.LENGTH_TYPE)];
		...
		/* xs:dateTime */
			...
			byte[] data = new byte[(Collection.LENGTH_COLLECTION_ID + ValueIndexFactory.LENGTH_TYPE) + 8];						//alocate a byte array for holding collectionId,Type,long (11 = (byte)short + byte + (byte)long)
		...
		else if(Type.subTypeOf(value.getType(), Type.INTEGER))
		{
	        ...
	        byte[] data = new byte[(Collection.LENGTH_COLLECTION_ID + ValueIndexFactory.LENGTH_TYPE) + 8];
			ByteConversion.shortToByte(collectionId, data, OFFSET_COLLECTION_ID);
		...
		{
	        final byte[] data = new byte[(Collection.LENGTH_COLLECTION_ID + ValueIndexFactory.LENGTH_TYPE) + 8];
	        ByteConversion.shortToByte(collectionId, data, OFFSET_COLLECTION_ID);
		...
		{
	        final byte[] data = new byte[(Collection.LENGTH_COLLECTION_ID + ValueIndexFactory.LENGTH_TYPE) + 4];
	        ByteConversion.shortToByte(collectionId, data, OFFSET_COLLECTION_ID);
		...
		{
			byte[] data = new byte[(Collection.LENGTH_COLLECTION_ID + ValueIndexFactory.LENGTH_TYPE) + 1];
	        ByteConversion.shortToByte(collectionId, data, OFFSET_COLLECTION_ID);
	...
	public final static byte[] serialize(Indexable value, int offset, boolean caseSensitive) 
		...
		{			
			final String val = caseSensitive ? 
				((StringValue)value).getStringValue() : 
				((StringValue)value).getStringValue().toLowerCase();
			final byte[] data = new byte[ offset + ValueIndexFactory.LENGTH_TYPE + UTF8.encoded(val) ];
			data[offset] = (byte) value.getType();	// TODO: cast to byte is not safe
			UTF8.encode(val, data, offset + ValueIndexFactory.LENGTH_TYPE);  
			return data;
		}
		...
		/* xs:dateTime */
	    	...
	    	GregorianCalendar utccal = ((AbstractDateTimeValue)value).calendar.normalize().toGregorianCalendar();	//Get the dateTime (XMLGregorianCalendar) normalized to UTC (as a GregorianCalendar)
			long millis = utccal.getTimeInMillis();									//Get the normalized dateTime as a long (milliseconds since the Epoch)
			final byte[] data = new byte[offset + ValueIndexFactory.LENGTH_TYPE + 8];		//allocate an appropriately sized byte array for holding Type,long
		...
		{
			final byte[] data = new byte[ offset + ValueIndexFactory.LENGTH_TYPE + 8];
			data[offset] = (byte) Type.INTEGER;
	        long l = ((IntegerValue)value).getValue() - Long.MIN_VALUE;
			ByteConversion.longToByte(l, data, offset + ValueIndexFactory.LENGTH_TYPE);
			return data;
		}
		...
		else if (value.getType() == Type.DOUBLE)
		{
	        ...
	        final long bits = Double.doubleToLongBits(((DoubleValue)value).getValue()) ^ 0x8000000000000000L;
	        ByteConversion.longToByte(bits, data, offset + ValueIndexFactory.LENGTH_TYPE);
	        return data;
		}
		...
		{
			final byte[] data = new byte[offset + ValueIndexFactory.LENGTH_TYPE + 4];
			data[offset] = (byte) Type.FLOAT;
	        final int bits = (int)(Float.floatToIntBits(((FloatValue)value).getValue()) ^ 0x80000000);
	        ByteConversion.intToByte(bits, data, offset + ValueIndexFactory.LENGTH_TYPE);
	        return data;
		}		
		...
		{
			final byte[] data = new byte[ offset + ValueIndexFactory.LENGTH_TYPE + 1];
	    	data[offset] = Type.BOOLEAN;
	        data[offset + ValueIndexFactory.LENGTH_TYPE] = (byte)(((BooleanValue)value).getValue() ? 1 : 0);
	        return data;
		}		
