 ...
 * meier@ifs.tu-darmstadt.de http://exist.sourceforge.net
 * 
...
public class NativeSerializer extends Serializer {
    ...
    protected void serializeToSAX(NodeSet set, int start, int howmany,
            long queryTime) throws SAXException {
        ...
        if (generateDocEvents) contentHandler.startDocument();
        contentHandler.startPrefixMapping("exist", EXIST_NS);
        AttributesImpl attribs = new AttributesImpl();
        attribs.addAttribute("", "hitCount", "hitCount", "CDATA", Integer
                .toString(set.getLength()));
        if (queryTime >= 0)
                attribs.addAttribute("", "queryTime", "queryTime", "CDATA",
                        Long.toString(queryTime));
        contentHandler
                .startElement(EXIST_NS, "result", "exist:result", attribs);
        NodeProxy p;
    ...
    protected void serializeToSAX(DocumentImpl doc, boolean generateDocEvent)
        ...
        for (int i = 0; i < children.getLength(); i++) {
            ...
            domIter.next();
            serializeToSAX(n, domIter, (DocumentImpl) n.getOwnerDocument(), n
                    .getGID(), false, null);
        }
        ...
        if (generateDocEvent) contentHandler.endDocument();
    }

    protected void serializeToSAX(NodeImpl n) throws SAXException {
        if (!(n instanceof NodeImpl))
                throw new RuntimeException("wrong implementation");
        serializeToSAX(new NodeProxy((DocumentImpl) n.getOwnerDocument(),
                ((NodeImpl) n).getGID()));
    }
    ...
    protected void serializeToSAX(NodeImpl node, Iterator iter,
            DocumentImpl doc, long gid, boolean first, Match match)
            throws SAXException {
        serializeToSAX(node, iter, doc, gid, first, new TreeSet(), match);
    }

    protected void serializeToSAX(NodeImpl node, Iterator iter,
            DocumentImpl doc, long gid, boolean first, Set namespaces,
            Match match) throws SAXException {
        setDocument(doc);
        ...
        case Node.ELEMENT_NODE:
            ...
            if ((first && showId == EXIST_ID_ELEMENT) || showId == EXIST_ID_ALL) {
                attributes.addAttribute(EXIST_NS, "id", "exist:id", "CDATA",
                        Long.toString(gid));
            }
            if (first && showId > 0) {
            ...
            while (count < children) {
                ...
                if (child.getNodeType() == Node.ATTRIBUTE_NODE) {
                    ...
                    attributes.addAttribute(child.getNamespaceURI(), child
                            .getLocalName(), child.getNodeName(), "CDATA",
                            cdata);
                    count++;
                    gid++;
                    child.release();
            ...
            contentHandler.startElement(ns, node.getLocalName(), node
                    .getNodeName(), attributes);
            while (count < children) {
                serializeToSAX(child, iter, doc, gid++, false, namespaces,
                        match);
                if (++count < children) {
                    child = (NodeImpl) iter.next();
                } else
            ...
            contentHandler.endElement(ns, node.getLocalName(), node
                    .getNodeName());
            if (((ElementImpl) node).declaresNamespacePrefixes()) {
                String prefix;
                for (Iterator i = ((ElementImpl) node).getPrefixes(); i
        ...
        case Node.TEXT_NODE:
            if (first && createContainerElements) {
                AttributesImpl attribs = new AttributesImpl();
                if (showId > 0) {
                    attribs.addAttribute(EXIST_NS, "id", "exist:id", "CDATA",
                            Long.toString(gid));
                    attribs.addAttribute(EXIST_NS, "source", "exist:source",
                            "CDATA", doc.getFileName());
                }
                contentHandler.startElement(EXIST_NS, "text", "exist:text",
                        attribs);
            }
        ...
        case Node.ATTRIBUTE_NODE:
            if (first && createContainerElements) {
                ...
                attribs.addAttribute(node.getNamespaceURI(), node
                        .getLocalName(), node.getNodeName(), "CDATA", cdata);
                contentHandler.startElement(EXIST_NS, "attribute",
                        "exist:attribute", attribs);
                contentHandler.endElement(EXIST_NS, "attribute",
                        "exist:attribute");
            } else {
    ...
    protected void serializeToReceiver(NodeProxy p, Receiver receiver)
    throws SAXException {
        Iterator domIter = broker.getNodeIterator(p);
        serializeToReceiver(null, domIter, p.doc, p.gid, p.match, receiver);
    }
    
    protected void serializeToReceiver(NodeImpl node, Iterator iter,
            DocumentImpl doc, long gid, Match match, Receiver receiver) 
    throws SAXException {
        if (node == null) node = (NodeImpl) iter.next();
        if (node == null) return;
        char ch[];
        ...
        switch (node.getNodeType()) {
        case Node.ELEMENT_NODE:
            receiver.startElement(node.getQName());
            ...
            while (count < children) {
                ...
                if (child.getNodeType() == Node.ATTRIBUTE_NODE) {
                    if ((getHighlightingMode() & TAG_ATTRIBUTE_MATCHES) > 0)
                        ...
                        cdata = ((AttrImpl) child).getValue();
                    receiver.attribute(child.getQName(), cdata);
                    count++;
                    gid++;
                    child.release();
                } else
            ...
            while (count < children) {
                serializeToReceiver(child, iter, doc, gid++, match, receiver);
                if (++count < children) {
                    child = (NodeImpl) iter.next();
                } else
                    break;
    ...
    private final void textToReceiver(String data, Receiver receiver)
        ...
        while (p0 < data.length()) {
            ...
            if (inTerm) {
                receiver.startElement(MATCH_ELEMENT);
    ...
    private final void scanText(String data) throws SAXException {
        ...
        while (p0 < data.length()) {
            ...
            if (inTerm) {
                contentHandler.startElement(EXIST_NS, "match", "exist:match",
                        EMPTY_ATTRIBUTES);
