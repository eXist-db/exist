...
public class NativeSerializer extends Serializer {
    ...
    protected void serializeToSAX( NodeSet set, int start, int howmany, long queryTime )
        ...
        contentHandler.endElement( "http://exist.sourceforge.net/NS/exist",
            "result",
            "exist:result" );
    ...
    protected void serializeToSAX( Document doc, boolean generateDocEvent )
        ...
        for ( int i = 0; i < children.getLength(); i++ ) {
            ...
            Iterator domIter =
                broker.getDOMIterator( (DocumentImpl) n.getOwnerDocument(),
                ( (NodeImpl) n ).getGID() );
            serializeToSAX( null, domIter, (DocumentImpl) n.getOwnerDocument(),
                ( (NodeImpl) n ).getGID(),
                false );
        }
        LOG.debug( "serializing document " + ((DocumentImpl)doc).getDocId() + "to SAX took " + 
    ...
    protected void serializeToSAX( NodeProxy p, boolean generateDocEvents )
        ...
        serializeToSAX( null, domIter, p.doc, p.getGID(),
            true );
        contentHandler.endPrefixMapping( "exist" );
        if ( generateDocEvents )
            contentHandler.endDocument();
     ...
     *@exception  SAXException  Description of the Exception
     */
    protected void serializeToSAX( NodeImpl node, Iterator iter, DocumentImpl doc,
                                   long gid, boolean first ) throws SAXException {
     ...
     */
    protected void serializeToSAX( NodeImpl node, Iterator iter, DocumentImpl doc,
                                   long gid, boolean first, ArrayList prefixes ) throws SAXException {
        setDocument( doc );
        if ( node == null ) {
        ...
        switch ( node.getNodeType() ) {
            case Node.ELEMENT_NODE:
                ...
                while ( count < children ) {
                    ...
                    if ( child.getNodeType() == Node.ATTRIBUTE_NODE ) {
                        attributes.addAttribute( child.getNamespaceURI(), child.getLocalName(),
                            child.getNodeName(),
                ...
                if ( first ) {
                    attributes.addAttribute( "http://exist.sourceforge.net/NS/exist", "id",
                        "exist:id", "CDATA", Long.toString( gid ) );
                    attributes.addAttribute( "http://exist.sourceforge.net/NS/exist", "source",
                        "exist:source", "CDATA", doc.getFileName() );
                }
                ...
                if ( ( (ElementImpl) node ).declaresNamespacePrefixes() ) {
                    ...
                    for ( Iterator i = ( (ElementImpl) node ).getNamespacePrefixes();
                        i.hasNext();  ) {
                ...
                String ns = defaultNS == null ? node.getNamespaceURI() :
                	defaultNS; 
                contentHandler.startElement( ns, node.getLocalName(),
                ...
                contentHandler.endElement( ns,
                    node.getLocalName(),
                    node.getNodeName() );
                if ( ( (ElementImpl) node ).declaresNamespacePrefixes() &&
                    myPrefixes != null ) {
                    String prefix;
                    for ( Iterator i = myPrefixes.iterator();
                        i.hasNext();  ) {
                        prefix = (String) i.next();
            ...
            case Node.ATTRIBUTE_NODE:
                if ( first && createContainerElements ) {
                    ...
                    attribs.addAttribute( "http://exist.sourceforge.net/NS/exist", "source",
                        "exist:source", "CDATA", doc.getFileName() );
                    attribs.addAttribute( node.getNamespaceURI(), node.getLocalName(),
                        node.getNodeName(), "CDATA", ( (AttrImpl) node ).getValue() );
            ...
            case Node.PROCESSING_INSTRUCTION_NODE:
                contentHandler.processingInstruction( ( (ProcessingInstructionImpl) node ).getTarget(),
