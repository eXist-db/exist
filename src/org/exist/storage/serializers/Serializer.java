...
public class Serializer implements XMLReader {
    ...
    public Serializer( DBBroker broker, Configuration config ) {
        ...
        String option = 
        	(String)config.getProperty("serialization.enable-xsl");
        if(option != null)
        	processXSL = option.equalsIgnoreCase("true");
        option =
        	(String)config.getProperty("serialization.enable-xinclude");
        if(option != null)
    ...
    /**
     *  Set the encoding. This will be written to the XML
	 *  declaration.
     *
	...
	/**
	 * Should XInclude elements be expanded?
	 */
    ...
    /**
     *  Sets the indent attribute of the Serializer object
     *
    ...
    protected StringWriter setPrettyPrinter( boolean xmlDecl ) {
        StringWriter sout = new StringWriter();
        OutputFormat format =
            new OutputFormat( "xml", encoding, indent );
	        ...
	        if ( stylesheet.indexOf( ":" ) > -1 ) {
	            ...
	            try {
	            	...
	            	if(stylesheet.indexOf('/') < 0 && doc != null)
	            		stylesheet = doc.getCollection().getName() + '/' + 
	            			stylesheet;
	                ...
	                if ( xsl.getCollection() != null )
	                    factory.setURIResolver( new InternalURIResolver( xsl.getCollection().getName() ) );
     ...
     *@exception  SAXException  Description of the Exception
     */
    public void toSAX( Document doc, boolean generateDocEvent )
         throws SAXException {
        if(processXSL) {
        	String stylesheet = hasXSLPi(doc);
         	if(stylesheet != null)
        ...
        setXSLHandler();
        serializeToSAX( doc, generateDocEvent );
    }
     ...
     *@exception  SAXException   Description of the Exception
     */
    public void toSAX( Node n, boolean generateDocEvents )
         throws SAXException {
        setXSLHandler();
        serializeToSAX( n, generateDocEvents );
    }
     ...
     *@exception  SAXException   Description of the Exception
     */
    public void toSAX( NodeProxy p, boolean generateDocEvents )
         throws SAXException {
        setXSLHandler();
    ...
    /**
     * Sets the createContainerElements.
     * @param createContainerElements The createContainerElements to set
     */
