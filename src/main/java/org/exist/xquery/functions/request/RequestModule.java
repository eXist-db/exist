...
public class RequestModule extends AbstractInternalModule {
    ...
    public void prepare(final XQueryContext context) throws XPathException {
        //TODO(AR) remove deprecated $request:request variable for eXist-db 5.0.0
        declareVariable(new DynamicVariable(REQUEST_VAR, () -> {
            final Optional<RequestWrapper> maybeRequest =
                    Optional.ofNullable(context.getHttpContext())
                            .map(XQueryContext.HttpContext::getRequest);
            if (maybeRequest.isPresent()) {
                try {
                    return XPathUtil.javaObjectToXPath(maybeRequest.get(), context);
