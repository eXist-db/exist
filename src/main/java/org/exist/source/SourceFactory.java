...
public class SourceFactory {
    ...
    public static @Nullable Source getSource(final DBBroker broker, final String contextPath, final String location, final boolean checkXQEncoding) throws IOException, PermissionDeniedException {
        ...
        if (source == null
                && (
                    (location.startsWith("file:/") || (contextPath != null && contextPath.startsWith("file:/")))
                    ||
                    ((!location.contains(":/")) && !(contextPath != null && contextPath.contains(":/")))
        )) {
            source = getSource_fromFile(contextPath, location, checkXQEncoding);

            // NOTE: ONLY needed in 4.x.x for API compatibility
            // TODO(AR) must not appear in 5.x.x versions
            if (source == null && ((!location.startsWith("/db")) && !(contextPath != null && contextPath.startsWith("/db")))) {
        ...
        try {
            ...
            if (doc != null) {
                if (doc.getResourceType() == DocumentImpl.BINARY_FILE) {
                    source = new DBSource(broker, (BinaryDocument) doc, true);
                } else {
                    try {
                        // XML document: serialize to string source so it can be read as a stream
                        // by fn:unparsed-text and friends
                        source = new StringSource(broker.getSerializer().serialize(doc));
