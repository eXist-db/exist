...
/**
 * Make sure that the configuration inside <code>installer/jobs.xml</code> references existing file names only
 * 
...
public class JobsXmlConfigurationTest {
  ...
  public void testIntegrityOfJobsXml() throws URISyntaxException, IOException {
    ...
    Files.readAllLines(jobsXmlPath).stream().forEach(line -> assertInstallationPath(line, repositoryRoot));
  }
