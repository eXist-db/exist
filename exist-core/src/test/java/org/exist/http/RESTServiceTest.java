...
public class RESTServiceTest {
    ...
    private static final String XML_WITH_XSLPI =
            "<?xml-stylesheet type=\"text/xsl\" href=\"" + TEST_XSLT_DOC_WITH_XSLPI_URI.lastSegment().toString() + "\"?>\n" +
    ...
    public static void setup() throws EXistException, PermissionDeniedException, IOException, SAXException, LockException {
        ...
        try (final DBBroker broker = pool.get(Optional.of(pool.getSecurityManager().getSystemSubject()));
            ...
            Collection col = null;
            try {
                col = broker.getOrCreateCollection(transaction, TEST_XSLPI_COLLECTION_URI);
                col.getLock().acquire(Lock.LockMode.WRITE_LOCK);

                IndexInfo indexInfo = col.validateXMLResource(transaction, broker, TEST_XSLT_DOC_WITH_XSLPI_URI, new StringInputSource(XSLT_WITH_XSLPI));
                col.store(transaction, broker, indexInfo, new StringInputSource(XSLT_WITH_XSLPI));

                indexInfo = col.validateXMLResource(transaction, broker, TEST_XML_DOC_WITH_XSLPI_URI, new StringInputSource(XML_WITH_XSLPI));
                col.store(transaction, broker, indexInfo, new StringInputSource(XML_WITH_XSLPI));

                broker.saveCollection(transaction, col);
            } finally {
                if (col != null) {
                    col.release(Lock.LockMode.WRITE_LOCK);
                }
            }
