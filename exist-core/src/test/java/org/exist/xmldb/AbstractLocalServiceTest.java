...
class AbstractLocalServiceTest {
    @Mock
    Subject user;
    @Mock
    ...
    TestLocalService service;

    @BeforeEach
    void prepare() {
        replay(collection);
        service = new TestLocalService(user, brokerPool, collection);
    }
    ...
    void setWrongCollectionType() {
        RemoteCollection remoteCollection = createMock(RemoteCollection.class);
        assertThatExceptionOfType(XMLDBException.class)
                .isThrownBy(() -> service.setCollection(remoteCollection))
                .withMessage("incompatible collection type: " + remoteCollection.getClass().getName())
    ...
    static class TestLocalService extends AbstractLocalService {
        TestLocalService(Subject user, BrokerPool brokerPool, LocalCollection collection) {
