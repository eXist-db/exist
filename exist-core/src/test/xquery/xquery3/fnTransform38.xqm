...
declare variable $testTransform:transform-38-xsl :=
...
declare variable $testTransform:transform-38-sections := document {
    <doc>
...
declare
    %test:assertTrue
function testTransform:transform-38() {
    let $xsl := $testTransform:transform-38-xsl
    let $sections := $testTransform:transform-38-sections
    let $result := fn:transform(map {"stylesheet-node":$xsl, "source-node":$sections,
            "base-output-uri" : resolve-uri("transform/sandbox/fn-transform-38/output.html",
            "file://Users/alan"), "delivery-format":"serialized"})
