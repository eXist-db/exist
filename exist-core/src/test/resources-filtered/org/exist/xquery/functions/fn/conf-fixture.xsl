...
<xsl:stylesheet version="3.0"
    ...
    <!-- FunEnvironmentTest's "secure" fixture: restrict fn:available-environment-variables#0 /
         fn:environment-variable#1 to the "admins" group, so a caller who is not a member of
         that group (even a DBA) is denied and the AccessUtil access-control path is exercised
         instead of the auto-registered, unrestricted default. -->
    <xsl:param name="extra-modules" as="element()*">
        <module xmlns="" uri="http://www.w3.org/2005/xpath-functions" class="org.exist.xquery.functions.fn.FnModule">
            <parameter name="environmentVariableAccess.*.requiresGroup" value="admins"/>
        </module>
    </xsl:param>
