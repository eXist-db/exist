...
<xsl:stylesheet version="3.0"
    ...
    <!--
        'http://exist-db.org/xquery/util' is intentionally NOT in this list: canonical's entry
        is a bare, self-closing <module>, but this fixture needs one with child <parameter>
        elements, supplied below via $extra-modules instead (keeping it here too would produce
        a duplicate registration for the same uri).
    -->
