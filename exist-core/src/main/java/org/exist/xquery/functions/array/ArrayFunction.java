...
public class ArrayFunction extends BasicFunction {
    ...
    public static final FunctionSignature EMPTY = functionSignature(
            Fn.EMPTY.fname,
            "Returns true if the supplied array is empty.",
            returns(Type.BOOLEAN, "true if the array is empty"),
            INPUT_ARRAY
    );
    public static final FunctionSignature FOOT = functionSignature(
            Fn.FOOT.fname,
            "Returns the last member of an array.",
            returnsOptMany(Type.ITEM, "The last member of the array"),
            INPUT_ARRAY
    );
    public static final FunctionSignature TRUNK = functionSignature(
            Fn.TRUNK.fname,
            "Returns all members except the last from a supplied array.",
            returns(Type.ARRAY_ITEM, "All members except the last"),
            INPUT_ARRAY
    );
    public static final FunctionSignature ITEMS = functionSignature(
            Fn.ITEMS.fname,
            "Returns the members of an array as a sequence.",
            returnsOptMany(Type.ITEM, "The members as a sequence"),
            INPUT_ARRAY
    );
    public static final FunctionSignature MEMBERS = functionSignature(
            Fn.MEMBERS.fname,
            "Returns each member of an array as a single-member array.",
            returns(Type.ARRAY_ITEM, Cardinality.ZERO_OR_MORE, "A sequence of single-member arrays"),
            INPUT_ARRAY
    );
    ...
    // --- XQuery 4.0 array function implementations ---

    private static Sequence empty(Sequence[] args) {
        final ArrayType array = (ArrayType) args[0].itemAt(0);
        return BooleanValue.valueOf(array.getSize() == 0);
    }

    private static Sequence foot(Sequence[] args) throws XPathException {
        final ArrayType array = (ArrayType) args[0].itemAt(0);
        if (array.getSize() == 0) {
            throw new XPathException((Expression) null, ErrorCodes.FOAY0001, "Array is empty");
        }
        return array.get(array.getSize() - 1);
    }

    private static ArrayType trunk(Sequence[] args) throws XPathException {
        final ArrayType array = (ArrayType) args[0].itemAt(0);
        if (array.getSize() == 0) {
            return array;
        }
    ...
    private static Sequence items(Sequence[] args) throws XPathException {
        final ArrayType array = (ArrayType) args[0].itemAt(0);
