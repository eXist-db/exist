...
public class FunDeepEqual extends CollatingFunction {
    ...
    public static int deepCompare(final Item item1, final Item item2, @Nullable final Collator collator) {
        ...
        try {
            if (item1.getType() == Type.ARRAY || item2.getType() == Type.ARRAY) {
            ...
            if (item1.getType() == Type.MAP || item2.getType() == Type.MAP) {
                if (item1.getType() != item2.getType()) {
                    return Constants.INFERIOR;
                }
            ...
            final boolean item1IsAtomic = Type.subTypeOf(item1.getType(), Type.ATOMIC);
            final boolean item2IsAtomic = Type.subTypeOf(item2.getType(), Type.ATOMIC);
