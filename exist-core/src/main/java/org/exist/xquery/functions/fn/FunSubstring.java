...
public class FunSubstring extends Function {
	...
	public Sequence eval(Sequence contextSequence, Item contextItem) throws XPathException
	{
		...
		if(seqSourceString.isEmpty()) {
        	...
        	final NumericValue startingLoc = ((NumericValue)(argStartingLoc.eval(contextSequence).itemAt(0).convertTo(Type.NUMERIC))).round();
        	if(!validStartPosition(startingLoc, sourceString.length())) {
				...
				if(getArgumentCount() > 2) {
                    ...
                    final NumericValue length = ((NumericValue)(argLength.eval(contextSequence).itemAt(0).convertTo(Type.NUMERIC))).round();
