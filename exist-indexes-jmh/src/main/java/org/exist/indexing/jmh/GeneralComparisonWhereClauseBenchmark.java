...
/**
 * Benchmarks Wolfgang's "prefer XPath predicates over where expressions"
 * advice (https://exist-db.org/.../articles/tuning) directly. The query
 * doesn't call a {@code range:eq()} or {@code ngram:contains()} explicitly --
 * it uses a plain general comparison ({@code = 'HAMLET'}) that the optimizer
 * is supposed to route through the range index when one is configured. The
 * four shapes mirror the per-index benchmarks so they're directly comparable.
 *
 * <p>This is the case Wolfgang's docs describe: the engine "will always try to
 * process a where clause like an equivalent XPath with predicate, but it only
 * detects the simple cases" -- this benchmark measures how detected vs.
 * undetected diverge in practice, and is the JMH evidence for whether the
 * WhereClause.accept fix in PR #6300 closes that gap for general comparisons
 * as well as for explicit indexed function calls.
 */
...
@Fork(1)
public class GeneralComparisonWhereClauseBenchmark {
    ...
    private static final String QUERY_LITERAL = """
            count(collection('%s')//SPEAKER[. = 'HAMLET'])
            """.formatted(COLLECTION_PATH);

    private static final String QUERY_LET_VAR = """
            let $q := 'HAMLET'
            return count(collection('%s')//SPEAKER[. = $q])
            """.formatted(COLLECTION_PATH);
    ...
    public void setUp() throws Exception {
        ...
        for (int i = 0; i < CORPUS_RESOURCES.length; i++) {
            try (InputStream in = GeneralComparisonWhereClauseBenchmark.class.getResourceAsStream(CORPUS_RESOURCES[i])) {
                if (in == null) {
        ...
        try (InputStream in = GeneralComparisonWhereClauseBenchmark.class.getResourceAsStream(CONFIG_RESOURCE)) {
            if (in == null) {
        ...
        try (final DBBroker broker = pool.get(Optional.of(pool.getSecurityManager().getSystemSubject()))) {
            ...
            if (hits == 0) {
                throw new IllegalStateException("Corpus loaded but [. = 'HAMLET'] produced 0 hits -- index not active?");
            }
        }
    }
