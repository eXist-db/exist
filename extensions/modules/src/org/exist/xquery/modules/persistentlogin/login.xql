...
(:~
    ...
    The function sets the request attributes "xquery.user" and "xquery.password" if the user
    is authenticated. The request attributes will be picked up by subsequent queries processing the
    same request.
    ...
    If not expiration date was set ($maxAge is empty), the function will fall back to the
...
:)
declare function login:set-user($domain as xs:string, $maxAge as xs:dayTimeDuration?, $asDba as xs:boolean) as empty() {
...
declare %private function login:get-credentials($domain as xs:string, $token as xs:string, $asDba as xs:boolean) as empty() {
    plogin:login($token, login:callback(?, ?, ?, ?, $domain, $asDba))
};
...
declare %private function login:create-login-session($domain as xs:string, $user as xs:string, $password as xs:string,
    ...
    if (exists($maxAge)) then
        plogin:register($user, $password, $maxAge, login:callback(?, ?, ?, ?, $domain, $asDba))
    else
        login:fallback-to-session($domain, $user, $password, $asDba)
};

declare %private function login:clear-credentials($token as xs:string?) as empty() {
    if ($token) then
...
(:~
 ...
 :)
declare function login:fallback-to-session($domain as xs:string, $user as xs:string, $password as xs:string?, $asDba as xs:boolean) {
