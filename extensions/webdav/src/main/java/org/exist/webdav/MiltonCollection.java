...
public class MiltonCollection extends MiltonResource
     ...
     */
    public MiltonCollection(final Properties configuration, final String host, final XmldbURI uri, final BrokerPool pool) {
     ...
     * @param pool    Handle to Exist database.
     */
    public MiltonCollection(final Properties configuration, final String host, final XmldbURI uri, final BrokerPool pool, final Subject subject) {
        super(configuration);
    ...
    public Resource child(final String childName) {
        ...
        final List<? extends Resource> allResources = getChildren();
    ...
    private List<MiltonCollection> getCollectionResources() {
        final List<MiltonCollection> allResources = new ArrayList<>();
        for (final XmldbURI path : existCollection.getCollectionURIs()) {
    ...
    private List<MiltonDocument> getDocumentResources() {
        final List<MiltonDocument> allResources = new ArrayList<>();
        for (final XmldbURI path : existCollection.getDocumentURIs()) {
            final MiltonDocument mdoc = new MiltonDocument(configuration, this.host, path, brokerPool, subject);
    ...
    public Date getCreateDate() {
        ...
        final Long time = existCollection.getCreationTime();
        if (time != null) {
    ...
    public CollectionResource createCollection(final String name)
        ...
        try {
            final XmldbURI collectionURI = existCollection.createCollection(name);
        ...
        } catch (final CollectionExistsException | EXistException ex) {
            LOG.debug(ex.getMessage());
     ...
     * =============== */
    @Override
    public Resource createNew(final String newName, final InputStream is, final Long length, final String contentType)
        ...
        try {
            // submit
            final XmldbURI resourceURI = existCollection.createFile(newName, is, length, contentType);
    ...
    public LockToken createAndLock(final String name, final LockTimeout timeout, final LockInfo lockInfo) throws NotAuthorizedException {
        ...
        final String token = UUIDGenerator.getUUIDversion4();
    ...
    public LockResult refreshLock(final String token) throws NotAuthorizedException, PreConditionFailedException {
        ...
        final LockInfo lockInfo = new LockInfo(LockInfo.LockScope.NONE, LockInfo.LockType.READ, token, LockInfo.LockDepth.ZERO);
        final LockTimeout lockTime = new LockTimeout(Long.MAX_VALUE);

        final LockToken lockToken = new LockToken(token, lockInfo, lockTime);
    ...
    @Override
    public void copyTo(final CollectionResource toCollection, final String newName) {

        if (LOG.isDebugEnabled()) {
            LOG.debug("Move '{}' to '{}' in '{}'", resourceXmldbUri, newName, toCollection.getName());
        }

        final XmldbURI destCollection = ((MiltonCollection) toCollection).getXmldbUri();
        try {
    ...
    public void sendContent(final OutputStream out, final Range range, final Map<String, String> params,
                            final String contentType) throws IOException, NotAuthorizedException, BadRequestException {

        try {
            final XMLOutputFactory xf = XMLOutputFactory.newInstance();
            xf.setProperty("javax.xml.stream.isRepairingNamespaces", Boolean.TRUE);

            final XMLStreamWriter writer = xf.createXMLStreamWriter(out);
            ...
            for (final MiltonCollection collection : getCollectionResources()) {
                collection.writeXML(writer);
            }

            // Iterate over all documents in collection
            for (final MiltonDocument document : getDocumentResources()) {
    ...
    private void writeXML(final XMLStreamWriter writer) throws XMLStreamException {
        writer.writeStartElement("exist", "collection", "http://exist.sourceforge.net/NS/exist");
        writer.writeAttribute("name", resourceXmldbUri.lastSegment().toString());
        writer.writeAttribute("created", getXmlDateTime(existCollection.getCreationTime()));
        writer.writeAttribute("owner", existCollection.getOwnerUser());
