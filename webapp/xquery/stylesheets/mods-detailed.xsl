    ...
    <xsl:template name="navigation">
        ...
        <tr class="result-head">
            <th class="nav{$position}" width="20%">
                <xsl:if test="@start &gt; 1">
                    <a class="mods" href="mods.xq?start={@start - 1}&amp;display=details&amp;max={@max}">
                        &lt;&lt; Previous
                    </a>
                </xsl:if>
            </th>
            <th class="nav{$position}" width="60%">
                <a class="mods" href="mods.xq?start={$summary}&amp;display=summary&amp;max={@max}">
                    <span class="icondesc">[Short Display]</span>
                </a>
                <a class="mods" href="mods.xml">
                    <span class="icondesc">[New Query]</span>
                </a>
            </th>
            <th class="nav{$position}" width="20%">
                <xsl:if test="number(@next) &lt; @hits">
                    <a class="mods" href="mods.xq?start={@next}&amp;display=details&amp;max={@max}">
                        Next &gt;&gt;
                    </a>
                </xsl:if>
            </th>
    ...
    <xsl:template match="m:name">
        ...
        <xsl:choose>
            <xsl:when test="m:namePart[not(@type)]">
                <a class="mods"
                    href="mods.xq?field1=au&amp;term1={java:java.net.URLEncoder.encode(m:namePart[not(@type)], 'UTF-8')}&amp;mode1=near&amp;max={ancestor::query-results/@max}">
            ...
            <xsl:when test="m:namePart[@type='family']">
                ...
                <a class="mods"
                    href="mods.xq?field1=au&amp;term1={java:java.net.URLEncoder.encode($name, 'UTF-8')}&amp;mode1=near&amp;max={ancestor::query-results/@max}">
        ...
        <xsl:if test="m:role">
            <xsl:text> (</xsl:text>
            <xsl:value-of select="m:role/m:roleTerm"/>
            <xsl:text>)</xsl:text>
        </xsl:if>
    </xsl:template>
