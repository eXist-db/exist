...
Foundation, Inc., 51 Franklin St, Fifth Floor, Boston, MA  02110-1301  USA
	-->
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:ev="http://www.w3.org/2001/xml-events" xmlns:xforms="http://www.w3.org/2002/xforms" xmlns:xhtml="http://www.w3.org/1999/xhtml" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:ajx="http://www.ajaxforms.net/2006/ajx" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" version="1.0">
    <xsl:output method="html" omit-xml-declaration="no" indent="no" doctype-public="-//W3C//DTD XHTML 1.0 Transitional//EN"/>
    ...
    <xsl:template match="xhtml:html | html">
        ...
        <html>
            ...
            <head>
                ...
                <script src="{$resourcesdir}xsltforms.js" type="text/javascript">
                    <xsl:text/>
                </script>
    ...
    <xsl:template match="xforms:itemset">
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <option>
                    <xsl:call-template name="genid"/>...Loading...</option>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
        ...
        <span>
            ...
            <xsl:call-template name="style">
                <xsl:with-param name="class">xforms-control xforms-<xsl:value-of select="local-name()"/>
                    <xsl:if test="$appearance='minimal' or local-name()='trigger' or local-name()='submit' or string(xforms:label)=''"> xforms-appearance-minimal</xsl:if>
                </xsl:with-param>
            </xsl:call-template>
            <span>
                <span>
    ...
    <xsl:template name="style">
        ...
        <xsl:if test="@class or $class != ''">
            <xsl:attribute name="class">
                <xsl:if test="@class">
                    <xsl:value-of select="concat(concat(@class, ' '), $class)"/>
                </xsl:if>
    ...
    <xsl:template name="toScriptBinding">
        ...
        <xsl:choose>
            <xsl:when test="@bind">new Binding(null, null, "<xsl:call-template name="toXPathExpr">
                    <xsl:with-param name="p" select="@bind"/>
                </xsl:call-template>")</xsl:when>
            <xsl:when test="$xpath != '' and $model != ''">new Binding("<xsl:call-template name="toXPathExpr">
                    <xsl:with-param name="p" select="$xpath"/>
                </xsl:call-template>", "<xsl:call-template name="toXPathExpr">
                    <xsl:with-param name="p" select="$model"/>
                </xsl:call-template>")</xsl:when>
            <xsl:when test="$xpath != ''">new Binding("<xsl:call-template name="toXPathExpr">
                    <xsl:with-param name="p" select="$xpath"/>
                </xsl:call-template>")</xsl:when>
            <xsl:otherwise>null</xsl:otherwise>
        </xsl:choose>
    ...
    <xsl:template name="xpath">
        ...
        <xsl:variable name="result">new XPath("<xsl:value-of select="$xp"/>",<xsl:value-of select="$xp2jsres2"/>
                ...
                <xsl:variable name="i">
                    <xsl:choose>
                        ...
                        <xsl:when test="contains('_ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz',$c)">
                            ...
                            <xsl:value-of select="string-length($t)"/>.,new StepExpr('child',new NodeTestName('<xsl:value-of select="substring-before($pt,':')"/>','<xsl:value-of select="substring-after($pt,':')"/>
                            <xsl:text>')</xsl:text>
                        </xsl:when>
                        <xsl:when test="starts-with($s,'*:') and contains('_ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz',substring($s,3,1))">
                            ...
                            <xsl:choose>
                                <xsl:when test="not(contains($t,':'))">
                                    <xsl:value-of select="string-length($t)+2"/>.,new StepExpr('child',new NodeTestName('*','<xsl:value-of select="$t"/>
                                    <xsl:text>')</xsl:text>
                                </xsl:when>
                                <xsl:otherwise>
    ...
    <xsl:template name="fctfullname">
        ...
        <xsl:choose>
            <xsl:when test="contains($fctname,':')">
                ...
                <xsl:choose>
                    <xsl:when test="//namespace::*[name()=$ns]">
                        <xsl:value-of select="//namespace::*[name()=$ns][1]"/>
                    </xsl:when>
                    <xsl:when test="//*[starts-with(name(),concat($ns,':'))]">
                        <xsl:value-of select="namespace-uri(//*[starts-with(name(),concat($ns,':'))][1])"/>
                    </xsl:when>
                    <xsl:when test="//@*[starts-with(name(),concat($ns,':'))]">
                        <xsl:value-of select="namespace-uri(//@*[starts-with(name(),concat($ns,':'))][1])"/>
                    </xsl:when>
    ...
    <xsl:template match="xforms:load" mode="script" priority="1">
        ...
        <xsl:choose>
            <xsl:when test="xforms:resource/@value">
                <xsl:call-template name="toScriptBinding">
                    <xsl:with-param name="p" select="xforms:resource/@value"/>
                </xsl:call-template>
            </xsl:when>
    ...
    <xsl:template match="xforms:submission" mode="script" priority="1">
        ...
        <xsl:choose>
            <xsl:when test="xforms:method/@value">
                ...
                <xsl:call-template name="toScriptBinding">
                    <xsl:with-param name="p" select="xforms:method/@value"/>
                    <xsl:with-param name="model" select="$idmodel"/>
                </xsl:call-template>
            </xsl:when>
            <xsl:otherwise>
    ...
    <xsl:template match="xforms:toggle" mode="script" priority="1">
        ...
        <xsl:choose>
            <xsl:when test="xforms:case/@value">
                <xsl:call-template name="toScriptBinding">
                    <xsl:with-param name="p" select="xforms:case/@value"/>
                </xsl:call-template>
            </xsl:when>
