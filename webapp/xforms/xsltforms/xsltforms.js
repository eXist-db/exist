...
XFSubmission.prototype.submit = function() {
	...
	if(this.method == "xml-urlencoded-post") {
		var outForm = (typeof document.createElementNS != 'undefined') ? document.createElementNS("http://www.w3.org/1999/xhtml","form") : document.createElement("form");
		outForm.setAttribute("method", "post");
		outForm.setAttribute("action", action);
		var txt = (typeof document.createElementNS != 'undefined') ? document.createElementNS("http://www.w3.org/1999/xhtml","input") : document.createElement("input");
...
Calendar.daysOfMoth = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];
...
var Core = {
    ...
    createElement : function(type, parent, content, className) {
				var el = (typeof document.createElementNS != 'undefined') ? document.createElementNS("http://www.w3.org/1999/xhtml",type) : document.createElement(type);
...
Schema.all = {"xsd" : "http://www.w3.org/2001/XMLSchema"};
	    ...
	    if (!document.addEventListener) {
			...
			if (!event.explicitOriginalTarget) {
				if (event.currentTarget) {
					effectiveTarget = event.target == event.currentTarget;
				}
			} else {
				effectiveTarget = event.explicitOriginalTarget == event.target;
			}
			if (effectiveTarget) {
...
XPathCoreFunctions = {
	...
	"namespace-uri" : new XPathFunction(false, XPathFunction.DEFAULT_NODESET, false,
		function(nodeSet) {
			return nodeSet.length === 0? "" : xmlNamespaceURI(nodeSet[0]); // ??? Wha is xmlNamespaceURI ?
		} ),
...
function xmlResolveEntities(s) {
    ...
    for (var i = 1; i < parts.length; ++i) {
        ...
        switch (rp) {
            ...
            default:
                var span = (typeof window.document.createElementNS != 'undefined') ? window.document.createElementNS("http://www.w3.org/1999/xhtml",'span') : window.document.createElement('span');
                span.innerHTML = '&' + rp + '; ';
