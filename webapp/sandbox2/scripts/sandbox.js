...
function resizeQueryBox(minimizeOnly) {
    ...
    if (document.quMaximized) {
        newHeight = 170;
        anim = new YAHOO.util.Anim(codeEditor, { height: { to: newHeight } });
        anim.animate();
        document.quMaximized = false;
        element.innerHTML = "Maximize";
    } else if (!minimizeOnly) {
        newHeight = (document.body.clientHeight - codeEditor.offsetTop - 50);
        anim = new YAHOO.util.Anim(codeEditor, { height: { to: newHeight } });
        anim.animate();
...
function doSave(saveAs) {
    if (saveAs || currentResource.isNew()) {
        ...
        dialog.closeEvent.subscribe(function (type, args) {
            ...
            YAHOO.util.Connect.asyncRequest('PUT', '/exist/rest' + args[1] + '/' + args[0], callback, codeEditor.getCode());
        });
        dialog.show();
    } else {
        ...
        YAHOO.util.Connect.setDefaultPostHeader(false);
        YAHOO.util.Connect.initHeader('Content-Type', currentResource.mimeType);
        YAHOO.util.Connect.asyncRequest('PUT', '/exist/rest' + currentResource.path, callback, codeEditor.getCode());
    }
}
...
function compileAndCheck() {
    ...
    var params = 'check=' + escapeQuery(codeEditor.getCode());
    var callback = {
        success: function (response) {
...
function switchSlot(node) {
	var id = node.id;
	var currentQuery = codeEditor.getCode();
