...
define("eXide/mode/behaviour/xml",function(a,b){var c=a("ace/lib/oop"),f=a("ace/mode/behaviour").Behaviour,d=a("ace/mode/behaviour/cstyle").CstyleBehaviour;a("eXide/mode/behaviour/xquery");var e=function(e){this.inherit(d,["braces","parens","string_dquotes"]);this.parent=e;this.add("brackets","insertion",function(d,e,a,f,b){return b=="\n"&&(e=a.getCursorPosition(),f.doc.getLine(e.row).substring(e.column,e.column+2)=="</")?(d=this.$getIndent(f.doc.getLine(e.row))+f.getTabString(),f=this.$getIndent(f.doc.getLine(e.row)),
{text:"\n"+d+"\n"+f,selection:[1,d.length,1,d.length]}):!1});this.add("slash","insertion",function(d,a,f,b,c){c=="/"&&(d=f.getCursorPosition(),a=b.doc.getLine(d.row),d.column>0&&a.charAt(d.column-1)=="<"&&(a=a.substring(0,d.column)+"/"+a.substring(d.column),f=b.doc.getAllLines(),f[d.row]=a,e.exec("closeTag",f.join(b.doc.getNewLineCharacter()),d.row)));return!1})};c.inherits(e,f);b.XMLBehaviour=e});
...
define("eXide/mode/html",function(a,b){var c=a("ace/lib/oop"),f=a("ace/mode/html").Mode,d=a("ace/tokenizer").Tokenizer,e=a("ace/mode/html_highlight_rules").HtmlHighlightRules,g=a("eXide/mode/behaviour/xml").XMLBehaviour,h=a("ace/range").Range,i=function(a){this.$tokenizer=new d((new e).getRules());this.$behaviour=new g(a)};c.inherits(i,f);(function(){this.toggleCommentLines=function(d,e,a,f){for(var b=!0,c=/^\s*<\!--(.*)--\>/,d=a;d<=f;d++)if(!c.test(e.getLine(d))){b=!1;break}for(var g=new h(0,0,0,
0),d=a;d<=f;d++)a=e.getLine(d),g.start.row=d,g.end.row=d,g.end.column=a.length,e.replace(g,b?a.match(c)[1]:"<\!--"+a+"--\>")}}).call(i.prototype);b.Mode=i});var eXide=eXide||{};eXide.namespace=function(a){var a=a.split("."),b=eXide,c;a[0]=="eXide"&&(a=a.slice(1));for(c=0;c<a.length;c++)typeof b[a[c]]=="undefined"&&(b[a[c]]={}),b=b[a[c]];return b};eXide.namespace("eXide.util");
...
this.$resolveImports(a,b)}},clearOutline:function(){$("#outline").empty()},$parseLocalFunctions:function(a,b){for(b.functions=[];;){var c=this.funcDefRe.exec(a);if(c==null)break;var f=this.funcDefRe.lastIndex,d=this.$findMatchingParen(a,f),e=c.length==3?c[2]:c[1];b.functions.push({type:"function",name:e,visibility:c.length==3?c[1]:"public",signature:e+"("+a.substring(f,d)+")"})}if(c=a.match(this.varDefRe))for(f=0;f<c.length;f++)d=this.varRe.exec(c[f]),b.functions.push({type:"variable",name:d[1]})},
$findMatchingParen:function(a,b){for(var c=1,f=b;f<a.length;f++){var d=a.charAt(f);if(d==")"){if(c-=1,c==0)return f}else d=="("&&(c+=1)}return-1},$parseImports:function(a){return a.match(this.parseImportRe)},$resolveImports:function(a,b){for(var c=this,f=[],d=[],e=0;e<b.length;e++){var g=this.moduleRe.exec(b[e]);g!=null&&g.length==4&&(d.push("prefix="+encodeURIComponent(g[1])),d.push("uri="+encodeURIComponent(g[2])),d.push("source="+encodeURIComponent(g[3])))}e="xmldb:exist://"+a.getBasePath();d.push("base="+
encodeURIComponent(e));$.ajax({url:"outline",dataType:"json",type:"POST",data:d.join("&"),success:function(d){if(d!=null){for(var d=d.modules,e=0;e<d.length;e++){var b=d[e].functions;if(b)for(var g=0;g<b.length;g++)f.push({type:"function",name:b[g].name,signature:b[g].signature,visibility:b[g].visibility,source:d[e].source});if(b=d[e].variables)for(g=0;g<b.length;g++)f.push({type:"variable",name:"$"+b[g],source:d[e].source})}a.functions=a.functions.concat(f);c.$outlineUpdate(a)}}});return f},$sortFunctions:function(a){a.functions.sort(function(a,
c){return a.name==c.name?0:a.name>c.name?1:-1})},$outlineUpdate:function(a){this.$sortFunctions(a);if(this.currentDoc==a){$("body").layout().open("west");eXide.app.resize();var b=$("#outline");b.empty();for(var c=0;c<a.functions.length;c++){var f=a.functions[c];$.log("Function %s visibility: '%s'",f.name,f.visibility);var d=document.createElement("li"),e=document.createElement("a");if(f.signature)e.title=f.signature;var g=$(e);f.type=="function"?g.addClass("t_function"):g.addClass("t_variable");e.href=
f.source?"#"+f.source:"#";f.visibility==="private"?g.addClass("private"):g.addClass("public");e.appendChild(document.createTextNode(f.name));d.appendChild(e);b.append(d);g.click(function(){var a=this.hash.substring(1);$(this).hasClass("t_function")?eXide.app.locate("function",a==""?null:a,$(this).text()):eXide.app.locate("variable",a==""?null:a,$(this).text());return!1})}}},$loadTemplates:function(){var a=this;$.ajax({url:"templates/snippets.xml",dataType:"xml",type:"GET",success:function(b){$(b).find("snippet").each(function(){var c=
$(this),b=c.attr("abbrev"),d=c.find("description").text(),c=c.find("code").text();a.templates.push({TYPE:"template",name:b,help:d,template:c})})}})}};return Constr}();eXide.namespace("eXide.edit.Document");
...
a.bottom)})};eXide.util.oop.inherit(Constr,eXide.events.Sender);Constr.prototype.setMode=function(a){this.mode=a;a=="manage"?this.grid.setOptions(c):this.grid.setOptions(b)};Constr.prototype.resize=function(){this.grid.resizeCanvas();this.grid.focus()};Constr.prototype.update=function(a,b){if(b||a!==this.collection)$.log("Opening resources for %s",a),this.collection=a,this.grid.invalidate(),this.data.length=0,this.grid.setActiveCell(0,1),this.grid.setSelectedRows([]),this.grid.onViewportChanged.notify(),
this.grid.focus()};Constr.prototype.load=function(a,b){var c=this;$.getJSON("modules/collections.xql",{root:this.collection,view:"r",start:a,end:b},function(g){for(var h=a;h<=b;h++)c.grid.invalidateRow(h);if(g&&g.items){c.data.length=g.total;for(h=0;h<g.items.length;h++)c.data[a+h]=g.items[h]}else c.data.length=0;c.grid.updateRowCount();c.grid.render();a==0&&(c.grid.setActiveCell(0,1),c.grid.focus())})};Constr.prototype.hasSelection=function(){var a=this.grid.getSelectionModel().getSelectedRows();
return a&&a.length>0};Constr.prototype.getSelected=function(){var a=this.grid.getSelectionModel().getSelectedRows();if(a.length==0)return null;for(var b=[],c=0;c<a.length;c++)b.push(this.collection+"/"+this.data[a[c]].name);return b};Constr.prototype.deleteResource=function(){var a=this.grid.getSelectionModel().getSelectedRows();if(a.length!=0){for(var b=[],c=0;c<a.length;c++)b.push(this.data[a[c]].name);var g=this;eXide.util.Dialog.input("Confirm Deletion","Are you sure you want to delete the selected resources?",
function(){$.log("Deleting resources %o",b);$.getJSON("modules/collections.xql",{remove:b,root:g.collection},function(a){g.reload();a.status=="fail"&&eXide.util.Dialog.warning("Delete Resource Error",a.message)})})}};Constr.prototype.cut=function(){this.clipboardMode="move";this.copy()};Constr.prototype.copy=function(){var a=this.grid.getSelectionModel().getSelectedRows();this.clipboard=[];for(var b=0;b<a.length;b++){var c=this.data[a[b]].name;c.substr(0,1)!="/"&&(c=this.collection+"/"+c);this.clipboard.push(c)}$.log("Clipboard: %o",
