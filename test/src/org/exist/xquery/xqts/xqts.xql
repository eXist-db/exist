...
declare variable $xqts:XQTS_HOME := xqts:path-to-uri(concat(system:get-exist-home(),"/",$xqts:CONFIG/basedir/text()));
...
declare function xqts:initialize() as element() {
    ...
    let $catalog := doc("/db/XQTS/XQTSCatalog.xml")
        ...
        (
            ...
            xdb:store($collection, "hacked-tests.xml",
                xs:anyURI(xqts:path-to-uri(
                        concat($home, "/test/src/org/exist/xquery/xqts/hacked-tests.xml")
                    ))) 
        )   
    return
        $config
...
declare variable $testGroup external;
declare variable $testCase external;
