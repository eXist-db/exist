...
public class IndexIntegrationTest extends AbstractTestUpdate {
    ...
    private interface SetupBlock {
        void run(IndexWorker worker, StreamListener stream);
    }

    private interface TestBlock {
        void run(XQueryService service) throws XMLDBException;
    }

    private void run(XmldbURI docUri, String data, SetupBlock setup, TestBlock test) throws Exception {
        XQueryService service = storeXMLStringAndGetQueryService(docUri.lastSegment().toString(), data);

        IMocksControl control = createStrictControl();

        IndexWorker worker = control.createMock(IndexWorker.class);
        StreamListener stream = control.createMock(AbstractStreamListener.class);

        AbstractIndex index = new TestIndex(worker);
    ...
    private static DocumentImpl eqDocument(XmldbURI url) {
    ...
    private static class DocumentMatcher implements IArgumentMatcher {
        ...
        public boolean matches(Object argument) {
            if (argument instanceof DocumentImpl) {
                DocumentImpl doc = (DocumentImpl)argument;
    ...
    private static AttrImpl eqAttr(String name, String value) {
    ...
    private static class AttributeMatcher implements IArgumentMatcher {
        ...
        public boolean matches(Object argument) {
            if (argument instanceof AttrImpl) {
                AttrImpl attr = (AttrImpl)argument;
