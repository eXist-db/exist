                    ...
                    <xsl:element name="{concat($functionName, 'Result')}" namespace="{$namespaceURL}">
                        <xsl:choose>
                            <xsl:when test="/webservice/functions/function[1]/return/cardinality &lt;= 4">
                                <!-- atomic value -->
                                <xsl:value-of select="/webservice/functions/function[1]/return/result/value"/>
                            </xsl:when>
                            <xsl:otherwise>
                                <!-- sequence of values -->
                                <xsl:for-each select="/webservice/functions/function[1]/return/result/sequence/value">
                                    <xsl:element name="{substring-after($type, ':')}" namespace="{$namespaceURL}"><xsl:value-of select="."/></xsl:element>
                                </xsl:for-each>
                            </xsl:otherwise>
                        </xsl:choose>
                    </xsl:element>
